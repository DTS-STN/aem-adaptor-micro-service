...
def map_home(aem_content: Any) -> HomeContent:
    ...
    homeContent = HomeContent(**{"title": aem_content["schPagev1ByPath"]["item"]["scTitleEn"], "cards": cards, "exitBeta": exitBeta})
...
def map_dashboard_cards(aem_cards: Any) -> List[Card]:
    ...
    for card in aem_cards:
        ...
        cards.append(Card(**{"id": card["scId"], "title": card["scTitleEn"], "lists": card_lists}))
    return cards
...
def map_list_tasks(aem_tasks: Any) -> List[Task]:
    ...
    for task in aem_tasks:
        tasks.append(Task(**{"title":task["scTitleEn"], "link": task["scDestinationURLEn"], "icon": task["scIconCSS"]}))
    return tasks

def map_exit_beta(aem_exit_beta: Any) -> ExitBeta:
    return ExitBeta(**{"title": aem_exit_beta["scTitleEn"], "link": aem_exit_beta["scDestinationURLEn"]})